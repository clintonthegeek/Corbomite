...
**Last updated:** 2026-04-15 — **Cluster H landed (all 6 phases).** `libs/core/` gained the menu-section/menu-event substrate (MenuSectionHelper + MenuEventEmitter) and the EditorSuggest dispatch framework (EditorSuggest base + EditorSuggestManager + HoverLinkSource registry). `src/editor/` ships HoverPopover (300ms wiki-link preview), WikiLinkSuggest, TagSuggest. `src/app/` ships RibbonSlot. `src/dialogs/` ships Notice. NoteEditorWidget refactored off hardcoded WikiLink/Tag enum onto the dispatch framework. FileExplorerPanel right-click menu now uses MenuSectionHelper + emits fileMenu mid-construction. 28 new unit tests across 5 test executables; all 67 in the full suite green.

---

## Current focus

**Cluster H landed (UI primitives + suggester framework).** Hover-link preview pops 300ms after a wiki-link hover; suggesters dispatch through insertion-order-first-wins; menus enforce the canonical section ordering. Plugin-facing surfaces (Cluster N) can now be populated. Recommended next: **Cluster I (MetadataCache parity)** stub-expansion (A+H both done — the parallel cache is the natural next step and unlocks property-call DSL + line/block/section operators), **Cluster F (Templates/Daily Notes)** stub-expansion, or **Cluster G full plan** for views/TextFileView contracts.
...
*(none — Cluster H complete 2026-04-15)*
