...
| Q.0 | Vault architecture refactor (Cluster Q prerequisite) | [full](2026-04-16-cluster-q0-vault-architecture.md) | Full | In progress (Phase 10 done 2026-04-17; Phase 9 next) | Collapses the three-way Vault split (`Corbomite::Vault` stub / `VaultModel` / `VaultService`) into a single `libs/vault/` library with `Vault` + `FileManager` + `TFile`/`TFolder`/`TAbstractFile`. Phases 1–8, 10 landed. Phase 9 (plugin proxy layer: VaultProxy + FileManagerProxy + PluginContext rewire) reordered after Phase 10 so proxies build against a clean canonical Vault. Spec at `../specs/2026-04-16-vault-architecture-design.md`. |
