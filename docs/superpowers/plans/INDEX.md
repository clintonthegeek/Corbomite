...
| Q.0 | Vault architecture refactor (Cluster Q prerequisite) | [full](2026-04-16-cluster-q0-vault-architecture.md) | Full | In progress (Phase 8 done 2026-04-17) | Collapses the three-way Vault split (`Corbomite::Vault` stub / `VaultModel` / `VaultService`) into a single `libs/vault/` library with `Vault` + `FileManager` + `TFile`/`TFolder`/`TAbstractFile`. Phases 1–8 landed (Phase 8: VaultService deleted, NoteService absorbed into VaultModel, RecentVaults helper, CorbomiteApp owns lifecycle). Phase 9 (plugin proxy layer rewrite: VaultProxy + FileManagerProxy + PluginContext rewire) is next. Spec at `../specs/2026-04-16-vault-architecture-design.md`. |
