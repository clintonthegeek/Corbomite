...
**Last updated:** 2026-08-20 — Cluster N Phases 0–4 landed on `feature/rich-clipboard` (N5 eyeball remains). Prior same-day: Cluster N opened. Prior: 2026-08-19 — Cluster M opened (canvas authoring parity, Graffodil-rebase-first). Prior: 2026-08-17 — Cluster L L0-L2 landed. Prior: 2026-06-10 — archive sweep.
...
| N | Rich clipboard (copy-as + smart paste) | [full](2026-08-20-cluster-n-rich-clipboard.md) | Full | **Phases 0–4 landed on `feature/rich-clipboard`. Next: N5 live eyeball.** | Parallel to Cluster M (isolated worktree — `master` is mid-M4). Markoff `ClipboardCodec` + canvas/Live/Source/Styled intercept + Corbomite Edit-menu Copy as / Paste as Plain (Markoff `365e9b91`). Punch-list HTML→MD ticked; image paste/drop still out of scope. **Post-reset N** — not legacy Cluster N (plugin-ready surfaces, closed 2026-04-17). |
...
- New cluster letters (A onwards) refer to the **post-reset** scheme. Legacy letters (anything in `archive/` or `decisions-archive.md` predating 2026-04-26) refer to the **pre-reset** scheme. Always qualify if ambiguous: "legacy Cluster Y" vs "Cluster Y (post-reset)". **Cluster N (this scheme) is rich clipboard, 2026-08-20.** Legacy Cluster N was plugin-ready surfaces (closed 2026-04-17).
