...
|---|---|---|---|---|---|
| D | Bases UI completion | [stub](2026-04-26-cluster-d-bases-ui-completion.md) | Stub | **In progress (D.1, D.2 done)** | **Unaffected by port.** Decomposed into sub-projects D.1–D.5. **D.1 (backend correctness)** + **D.2 (read-side rendering)** shipped 2026-05-25. D.1: VaultResolver seam + `file()`/`asFile`/`linksTo`, one-directional tags, sort pinned ([spec](../specs/2026-05-25-cluster-d1-bases-backend-correctness-design.md)/[plan](2026-05-25-cluster-d1-bases-backend-correctness.md)). D.2: `QTreeView`+`BasesTreeModel` collapsible groups + summary cells, multi-key header sort (`cycleHeaderSort` + `BasesHeaderView` indicators), Icon/Image/HTML cells ([spec](../specs/2026-05-25-cluster-d2-bases-read-side-rendering-design.md)/[plan](2026-05-25-cluster-d2-bases-read-side-rendering.md)) — *visual rendering pending user verification*. Remaining: D.3 editing UI (toolbar Sort+group/Properties/Views menus, formula editor, filter builder, properties drawer, inline cell edit), D.4 interactivity/export, D.5 plugin API. Markdown cells frozen on read-only-Live. |
