...
**Last updated:** 2026-08-17 — Cluster L (workspace/KDDW stabilization & nativization) opened. Prior: 2026-06-10 — archive sweep: 20 executed/obsolete plans + 17 executed/obsolete specs moved to `archive/` per audit; cluster banners added (obsolete G/H/J disposition, kept-stub D/E/I corrections).
...
| K | Markoff canvas leaf adoption | [full](2026-08-15-cluster-k-markoff-canvas-adoption.md) | Full | **In progress — Phase 0–1 done, Phase 2 done, Phase 3 partial (5/14 dogfood findings fixed 2026-08-17)** | Markoff's canvas production arc (D5 part 1) closed 2026-08-15 at 315/315 tests, pending its own G2 (Corbomite adoption) gate — this cluster is that adoption. Goal: swap `Markoff::Live::EditorWidget` (QML) for `Markoff::Canvas::EditorWidget` behind a settings toggle, verify parity, dogfood, then retire the QML leaf (mirrors Markoff's G2/G3). `NoteEditorWidget`'s leaf-agnostic base dispatch (contract-v2, closed 2026-06-10) means most call sites need zero changes. Remaining Phase 3 gap-fix work + Phase 4 (dogfood) + Phase 5 (retirement) still open — see punch-list `[cluster-k]` tag for the 9 unfixed findings. |
| L | Workspace/KDDW stabilization & nativization | [full](2026-08-17-cluster-l-workspace-stabilization.md) | Full | **Not started — evaluation done, phases ready** | Full re-evaluation of the tab/dock layer (2026-08-17): architecture sound, but three disease clusters remain — teardown fragility (crash engine: `deserialize`'s bare `qDeleteAll`, four inconsistent leaf-destruction disciplines), persistence split-brain (production save drops `floating`/`lastOpenFiles`; `_corbomite` keys in the Obsidian-shared workspace.json; empty node ids, no `dimension`), and Obsidian-literalism residue (nullptr-returning shells, dead writers, undebounced mirrors). Doctrine: compat at the file-format boundary, native Qt inside. Phase L1 (crash-safety) should precede K Phase 4 dogfooding. |
