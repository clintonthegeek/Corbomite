...
| N | Rich clipboard (copy-as + smart paste) | [full](2026-08-20-cluster-n-rich-clipboard.md) | Full | **Opened 2026-08-20. N0–N2 + Live codec + host Edit-menu in progress on `feature/rich-clipboard`. Next: N3 Source/Styled intercept, then N5 eyeball.** | Parallel to Cluster M (isolated worktree — `master` is mid-M4). Markoff-core `ClipboardCodec` + canvas leaf multi-flavor copy / HTML→MD paste landed (tests green, uncommitted). Source/Styled native intercept and live eyeball remain. Absorbs punch-list P5 paste-HTML→MD. Image paste/drop out of scope. **Post-reset N** — not legacy Cluster N (plugin-ready surfaces, closed 2026-04-17). |
