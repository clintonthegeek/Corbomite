...
**Last updated:** 2026-05-28 — Cluster D Filter Builder **shipped** ([spec](../specs/2026-05-27-cluster-d-filter-builder-design.md)/[plan](2026-05-27-cluster-d-filter-builder.md)). 4 feature commits (`73e67498..9591ce55`) via subagent-driven TDD. **Only D.5 (plugin API) remains.** Also today: `[canvas][interop]` P0 bundle drained (4 fixes — unknown-field passthrough, default-omission, V5 self-heal, `qRound` geometry). (Prior: foundation port reconciliation — Markoff QML/D2 rebuild merged at `v0.7.0-freeze`, obsoleting G/H/J, re-scoping E; D/F/I-Workspace unaffected. See "Obsoleted by the foundation rewrite" table below.)

## Active clusters

| Cluster | Title | Plan file | Type | Status | Notes |
|---|---|---|---|---|---|
| D | Bases UI completion | [stub](2026-04-26-cluster-d-bases-ui-completion.md) | Stub | **In progress (D.1–D.4 + formula editor + filter builder done; D.5 remains)** | **Unaffected by port.** Decomposed into sub-projects D.1–D.5. **D.1 (backend correctness)** + **D.2 (read-side rendering)** + **D.3 (toolbar menus + properties drawer)** shipped 2026-05-25; **D.4a (cell interactivity)** + **D.4b (export/copy + +New)** shipped 2026-05-26; **D.4c (undo/redo for value edits)** shipped 2026-05-27. D.1: VaultResolver seam + `file()`/`asFile`/`linksTo`, one-directional tags, sort pinned ([spec](../specs/2026-05-25-cluster-d1-bases-backend-correctness-design.md)/[plan](2026-05-25-cluster-d1-bases-backend-correctness.md)). D.2: `QTreeView`+`BasesTreeModel` collapsible groups + summary cells, multi-key header sort (`cycleHeaderSort` + `BasesHeaderView` indicators), Icon/Image/HTML cells ([spec](../specs/2026-05-25-cluster-d2-bases-read-side-rendering-design.md)/[plan](2026-05-25-cluster-d2-bases-read-side-rendering.md)). D.3: `ViewConfigOps` pure mutators + Properties/Sort+group/Views `Qt::Popup` toolbar panels + `PropertiesDrawer` (per-row frontmatter editor in a `QSplitter`) + inline-edit polish ([spec](../specs/2026-05-25-cluster-d3-bases-toolbar-menus-design.md)/[plan](2026-05-25-cluster-d3-bases-toolbar-menus.md)). D.4a: pure `CellHitTest` geometry + delegate `editorEvent` routing (link navigate / checkbox toggle / tag search / URL open), right-click file context menu, `[[wikilink]]` drag mime ([spec](../specs/2026-05-26-cluster-d4a-bases-cell-interactivity-design.md)/[plan](2026-05-26-cluster-d4a-bases-cell-interactivity.md)). D.4b: pure `TableExporter` (CSV/TSV/Markdown/HTML/`obsidian/table`) + `NewItemSeed` (AND-equality seeding) + `Formula::ast()`; `BasesView` Results menu (Copy table 4-MIME + Export CSV) + +New seeded create/open/rename ([spec](../specs/2026-05-26-cluster-d4b-bases-export-new-entry-design.md)/[plan](2026-05-26-cluster-d4b-bases-export-new-entry.md)). D.4c: per-view stale-guarded `QUndoStack` — `CmdSetFrontMatter` does all I/O inside one `processFrontMatter` mutator (race-free drift check; skip+notify+neutralize on external change), model/drawer emit `frontMatterEditRequested` → `BasesView::pushFrontMatterEdit` chokepoint, `MainWindow` Undo/Redo routes to the active `BasesView` ([spec](../specs/2026-05-27-cluster-d4c-bases-undo-design.md)/[plan](2026-05-27-cluster-d4c-bases-undo.md)) — *D.2/D.3/D.4a/D.4b/D.4c visual + interactive verification (incl. real Ctrl+Z + drift banner) pending user eyeball*. Formula editor: reusable `FormulaInput` (parse-validity + flat token autocomplete) + `FormulaEditDialog` + extended `PropertiesMenuPanel` (Add-formula/edit/delete/summary picker) + `BasesView` wiring (owns mutation via `FormulaOps`→`requestSave`) + `SummaryContext` custom-summary eval ([spec](../specs/2026-05-27-cluster-d-formula-editor-design.md)/[plan](2026-05-27-cluster-d-formula-editor.md)) — *autocomplete popup + dialogs pending user eyeball*. **Filter builder shipped 2026-05-28** ([spec](../specs/2026-05-27-cluster-d-filter-builder-design.md)/[plan](2026-05-27-cluster-d-filter-builder.md)): pure `FilterSpec` value tree + `fromFilter`/`toFilter` bridging the immutable `FilterNode` (single-child And/Or collapse via `optimize()`, Not preserved, blank-leaf drop); `FilterBuilderWidget` recursive group editor reusing `FormulaInput`; `FilterBuilderDialog` scope toggle (per-view + global) with OK gated on both valid; `BasesView` Filters toolbar button → `openFiltersDialog` → public `applyFilterSpecs` → `onConfigMutated` — *nested builder rendering + dialog flow pending user eyeball*. Remaining: **D.5 plugin API**. Hover-preview + Markdown cells frozen on read-only-Live. |
