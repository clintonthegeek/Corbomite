...
**Status:** **CLOSED and MERGED to `master` 2026-08-20 (Phases 0–5).** Live eyeball passed on the worktree binary. Markoff tip `bafa3095` (includes blockquote HTML/RTF export fix). A pre-merge cleanup pass found and fixed three real bugs the eyeball missed (incomplete LO-table-paste fix, BlockQuote/ListItem marker asymmetry in canvas and Styled) and ported the host wiring onto Cluster O's `MarkdownViewActions` (landed on `master` after this branch was cut) — see `decisions-archive.md`. Image paste/drop remains out of scope.
