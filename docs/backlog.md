...
### Menubar items don't dispatch their actions (split_right / split_down + likely others) — KXmlGui-rebuild regression
- **Source:** discovered 2026-04-24 during Cluster Y Phase 1 manual QA. User pressed `Ctrl+Shift+Right` (works fine — `duplicateLeaf` runs and returns new leaf) but clicking "Split Right" in the menubar does nothing — the `QAction::triggered` signal never fires for the menu path. Same `QAction` object; keyboard shortcut path works, menu path does not.
- **Blocks:** menubar UX (any menu item that depends on `KActionCollection` action-references surviving a `KXmlGui` rebuild)
- **Scope:** medium — root-cause investigation + likely a single-place fix
- **Suspected cause:** `62aeced5 fix(app): create ribbon toolbar after setupGUI to survive KXMLGUI rebuild` (and adjacent Cluster V / R commits) introduced reasoning about KXmlGui rebuilds reconstructing the menubar. If the menu entries reference *fresh* `QAction` instances after rebuild rather than the originals registered in `KActionCollection`, those fresh actions are unconnected. Connection-after-rebuild was hand-fixed for the ribbon toolbar; menubar may have an analogous gap.
- **Verification path:** confirm with `qDebug()` in the action's `triggered` lambda — keyboard prints, menu does not. Then inspect `MainWindow::setupGUI` / `KXMLGUIClient::createGUI` / how menu .rc-driven action lookup reaches `KActionCollection`.
- **Likely scope:** `split_right`, `split_down`, `tab_undo_close`, plus probably any other action registered around the same code path. Audit needed.
- **Out of scope for Cluster Y** — this is a Cluster V / R follow-up, not part of the Workspace substrate migration. Unblocks: better menubar UX; cluster Y is unaffected because it does not touch action wiring.
