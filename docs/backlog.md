...
### ~~Cluster Y — Workspace migration onto KDDockWidgets~~ Done 2026-04-25 — Closed across 8 phases (43 commits `fd336369..bd1b50aa`). Workspace substrate is now `KDDockWidgets::QtWidgets::MainWindow` + `DockWidget`; `.obsidian/workspace.json` byte-compat round-trip via Corbomite-owned `WorkspaceSerializer` against 9 fixtures (LayoutSaver unused). Tab drag / split / floating-window / popout all KDDW-driven. Absorbs G follow-ups #3 (`openLinkText` dispatcher) and #6 (`WorkspaceWindow` popout). Retro at [`cluster-retros/cluster-y.md`](cluster-retros/cluster-y.md). 5 carry-forward follow-ups landed under §3 below + Stability §10.
...
- **Scope:** large
- **Details:** Phases 1 (vendor) and 2 (`Corbomite::SourceEditor` shim + visual-line float scroll) landed 2026-04-15. **Phase 3 (public find/replace API)** absorbed into Markoff Phase C work-unit **C7** on 2026-04-20 — `Markoff::Source` owns that surface now. Phases 4–8 (fractional scroll ±0.5 line precision, syntax-highlight grammar registration, gutter fold arrow, cursor-column preservation across Source↔LivePreview, shared `VaultResourceProvider` promotion) move with `Markoff::Source` maintainership; no longer cluster-numbered on the Corbomite side. Track via Markoff's own backlog under the Source library.
...
### ~~Hamburger-menu "Split right" / "Split down" dispatch no-op — CommandRegistry / KActionCollection split~~ Fixed 2026-04-24 — originally filed as "menubar items don't dispatch" but diagnosis showed the menubar path works (QAction pointer matches `actionCollection()->action(id)`, `trigger()` correctly invokes the slot and `Workspace::duplicateLeaf` runs). The real break was the per-view hamburger "…" overflow in the view header: `MarkdownView::onMoreOptionsMenu` / `CanvasFileView::onMoreOptionsMenu` / `GraphView::onMoreOptionsMenu` dispatch `split_right` / `split_down` via `m_commandRegistry->executeById`, but those ids were only registered in `KActionCollection` (main menubar + keyboard shortcut), never in the `CommandRegistry` — `executeById` returned `false` silently. Fix registers the two ids as `CommandRegistry` commands in `MainWindow` ctor whose `checkCallback` delegates to the KAction's `trigger()` (keeps the vault-open gate from `updateVaultActions` as the single source of truth). Regression test `hamburgerSplitDispatchesCreateNewLeaf` in `tests/app/tst_mainwindow_action_wiring.cpp`. (Ribbon-toolbar fix 62aeced5 was unrelated — it addressed toolbar rebuild survival, not menu dispatch.)

### Tab drag-to-reorder within a tab bar is broken under Wayland — likely Qt platform issue
- **Source:** discovered 2026-04-24 during Cluster Y Phase 1 manual QA on Manjaro KWin Wayland. `QTabBar::setMovable(true)` is set at `libs/core/src/WorkspaceTabs.cpp:29` (unchanged in Cluster Y) but tabs cannot be dragged to reorder. Terminal prints "This plugin supports grabbing the mouse only for popup windows" — Wayland's protocol forbids non-popup mouse-grab, which is what `QTabBar` uses for drag-reorder.
- **Blocks:** in-tab-bar reorder UX on Wayland; nothing on X11 (untested)
- **Scope:** small — likely either a Qt 6.x version-pin question or a platform-conditional workaround
...
- **2026-04-25 update (Cluster Y closed):** P4b substrate flip is in. Tab bar is now KDDW's `TabBar` (no longer `QTabBar`). **Hypothesis:** this closes incidentally. Awaiting user manual QA of Y Task 8.4 to confirm. If broken, re-file as a Y P4b residue rather than a Cluster G era item.
...
- **2026-04-25 update (Cluster Y closed):** Y P4b substrate flip + P7.2 `getLeaf(LeafMode::Split, LeafDirection::Vertical)` factory both shipped. **Hypothesis:** the hamburger commands now route through the canonical factory rather than the legacy split path; quirk closes incidentally. Awaiting user manual QA of Y Task 8.4 to confirm. If still broken, requires a `duplicateLeaf` audit against the new substrate.
...
### Real `LinkResolverFn` wiring on `Workspace` (Cluster Y P7.3 carry-forward)
- **Source:** Cluster Y P7.3 closeout (`decisions-archive.md` 2026-04-25); `cluster-retros/cluster-y.md` open follow-up #1
- **Blocks:** Cluster R "Open linked view" submenu upgrade (currently dispatches to plugin `:open` commands rather than real leaf-opening); Cluster S `BookmarksView::onActivated` `#subpath` navigation (currently falls back to `openFile(path)` — see §1 Cluster S follow-up)
- **Scope:** small
- **Details:** `Workspace::openLinkText` ships with a `std::function<QString(linktext, source)>` resolver and an identity default. No production caller wires the lambda yet — `MainWindow::Setup` should install one that captures `Vault*` + `MetadataCache*` + `FileManager*`, calls `MetadataCache::resolveLink(linktext, source)`, and falls through to `FileManager::createFile` on miss (Obsidian's `Vault.create` semantic). Alternative: absorb into Cluster Z brainstorm — Z is the natural caller because linked-leaves drive most `openLinkText` traffic. Either home is fine; `Workspace::setLinkResolver(...)` is the single setter to call.
...
### ~~Word-processor-style Format/Heading checkstate + contextual menu gating~~ Absorbed into Markoff Phase C work-unit **C6** on 2026-04-20. Requirements spec recovered from the Corbomite submodule's ahead-master and reinstated in Markoff at `libs/markoff-family/libs/markoff-live/docs/specs/2026-04-20-consumer-editor-state-surface.md` (tag `v0.2.8`). Corbomite-side adaptation (retiring the "static enable, never checked" Format-toolbar state in `refreshEditorActions`) ships as C6's Corbomite beat. See `PROJECT-STATE.md` §Markoff Phase C.

### ~~Markoff editor right-click menu enrichment (`editor-menu` parity)~~ Absorbed into Markoff Phase C work-unit **C6** on 2026-04-20 (same spec §9). `aboutToShowContextMenu(QMenu*, EditorContext, QPoint)` signal is in scope for C6; Corbomite-side wrap of the menu in `MenuSectionHelper` + plugin `editor-menu` hook wiring ships as C6's Corbomite beat. See `PROJECT-STATE.md` §Markoff Phase C.
...
- **Scope:** small
- **Status:** **stays on Corbomite side** — this is KActionCollection / KXMLGUI plumbing that lives in `MainWindow`, not in Markoff. (Markoff Phase C C6's `EditorContext` + context-menu surface does *not* replace this item; they're complementary.) Pick up independently after Markoff C6 lands.
...
### Find/Replace state persistence across editor view modes
- **Source:** brainstorm 2026-04-21 after Markoff Phase C7 closed; user observation that search bar visibility, query, flags, and highlights do not carry across Source ↔ LivePreview ↔ Reading mode swaps.
- **Blocks:** nothing; UX polish only. Low priority.
- **Scope:** medium (touches Live, `NoteEditorWidget`, and Reading's search path).
- **Details:** Today each leaf owns its own `SearchBar` widget and its own state; `NoteEditorWidget::setViewMode` flips the `QStackedWidget` index so the other leaf's bar is a different hidden widget tree with separate state. Deeper than that, the three leaves don't share a search engine: Source (post-C7) runs on `Markoff::ReplaceController` + `SourceSearchAdapter`; Live still uses its legacy `Editor::findText` / `highlightAllMatches` / `replaceText` path with `LiveSearchAdapter` as a thin bridge; Reading has no `SearchBar` and `ReadingSearchAdapter::highlightMatches` is a Phase-A no-op stub. The tri-view spec (`libs/markoff-family/docs/specs/2026-04-20-tri-view-unified-api-design.md §Search + replace`) already describes the target shape — one `SearchController`, each view supplies a `SearchAdapter`, `SearchBar` in core as replaceable default UI — but marks persistence as "consumer concern," i.e. Corbomite-side. The clean target is: lift `SearchBar` from per-leaf to `NoteEditorWidget`, construct one `SearchController` per doc, each leaf only contributes a `SearchAdapter` for highlight rendering; `NoteEditorWidget::setViewMode` rebinds the controller's adapter without touching query/flags/current-index. Three prerequisites, all previously deferred:
  1. **Migrate `Markoff::Editor` (Live) off legacy `findText` / `replaceText` / `replaceAll` onto `SearchController`.** C7 spec §3 "Out" defers this: *"revisit naturally when C2 or C4 forces a pass through that file."* Until then Live's match-index ordering won't cleanly agree with Source's.
...
- **Blocks:** nothing hard; UX polish on top of C2.
- **Scope:** medium (needs Live painter-site + ReadingView CSS/palette coverage audit).
- **Details:** C2 shipped `Corbomite::Core::ThemeService` driving `editor->setThemeService(...)` on every `NoteEditorWidget` leaf, and `Markoff::Source::SourceEditor::setViewTheme` correctly bridges the theme through to Qutepart (palette + base font + current-line color). However the Live (`Markoff::Editor`) and Reading (`Markoff::Reading::ReadingView`) leaves visibly keep their default colours when the user picks a non-default theme from SettingsDialog → Editor theme. Live has ~20 painter sites consuming `m_theme.materializeFormat()` (per C2 activity log) and Reading consumes `Markoff::Theme` via `CodeBlockHighlighter`, so the theme *data* is reaching them — the gap is either (a) `setTheme` plumbing through `NoteEditorWidget::applyThemeToAllLeaves` not reaching the leaf instance at mount time, (b) a painter-site still reading a hardcoded default, or (c) the Reading CSS path still using its pre-C2 palette. Diagnose with logging on `Live::Editor::setTheme` / `Reading::ReadingView::setTheme` entry before widening the painter-site audit.
- **Deferred to:** natural slot is alongside C4 (renderer unification) if C4 forces a pass through the Live + Reading code-block/math/mermaid paths; otherwise standalone follow-up.

### ~~`WorkspaceWindow` popout integration~~ Done 2026-04-25 — `Workspace::popoutLeaf` shipped in Cluster Y P5 (`d84db521..967e34a5`) atop `KDDockWidgets::Core::FloatingWindow`. Geometry + maximize round-trip in `workspace.json` `floating[]` array; close-window cascades to leaf closes via `FloatingWindow::aboutToBeDestroyed` → `leafClosed`. Cluster R's "Open in new window" hamburger slot now activates. Per-window zoom deferred to Cluster V.2 (see §2 "Per-window zoom persistence"). Standalone `WorkspaceWindow` QWidget facade dead-code cleanup is its own carry-forward — see §3 entry below.
