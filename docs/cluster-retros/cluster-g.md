...
## Part 3 — Split semantics + active-pane focus routing (landed 2026-04-18)

A side-by-side audit of our Workspace/View stack against `docs/obsidian-audit/domains/workspace.md` + `domains/views.md`, triggered by a "Split Right leaves empty pane" bug report, confirmed the class hierarchy is 1:1 faithful to Obsidian's but surfaced several gaps. The P0 subset landed in one session as Part 3 (plan at `docs/superpowers/plans/2026-04-18-cluster-g-part3-split-semantics.md`):

- **`Workspace::duplicateLeaf(leaf, direction)`** — user-facing split primitive that snapshots `getViewState` + `getEphemeralState` + `history` + `pinned` + `group`, calls the existing `splitLeaf` tree op, creates a new leaf in the new sibling tabs, re-applies the snapshot, and sets the new leaf active. MainWindow's Split Right / Split Down actions rebind onto `duplicateLeaf`. `splitLeaf` stays as the low-level primitive (still used by the `WorkspaceController` plugin-facing proxy).
- **First-render show nudge.** `WorkspaceSplit::addChild` now calls `w->show()` after `QSplitter::insertWidget`. Qt's `insertWidget` only auto-shows widgets that were previously visible; widgets freshly reparented via `setParent(nullptr)` (as happens during `splitLeaf`'s re-wrap) stay hidden without the explicit show. This was the root cause of "main view goes blank on first Split Right; reappears only after vault reopen."
- **Focus-based active-leaf routing.** `Workspace` now listens to `QApplication::focusChanged` and walks the parent-widget chain of the newly-focused widget to find the owning leaf, then calls `setActiveLeaf`. This gives each split pane the Obsidian per-pane focus semantics — "open note" routes to whichever pane the user was last editing in.

4 new unit tests in `tst_workspace_integration` cover the return value, activation side-effect, pinned/group clone, and new-tabs contents. The non-P0 residue (empty-state `tD` view, unknown-viewType `nD` fallback, centralised `Workspace::openLinkText`, `FileView::receiveSyncState` linked-pane hook, popout-window integration, `ViewRegistry` error paths, `View.onTabMenu` default, `lastOpenFiles` load-side restore) is enumerated under "Cluster G follow-ups" in `docs/PROJECT-STATE.md` and remains deferred.
